...
{
	char extra_c = 0, padd = ' ';
	int ind = BUFF_SIZE - 2, length = 2, padd_s = 1; /* length=2, for '0x' */
	...
	return (write_p(buffer, ind, length,
		width, flags, padd, extra_c, padd_s));
}

/**
 * print_non - prints ascii codes in hexadecimal of non printable characters
 * @types: the list  of arguments
 * @buffer: the buffer array to handle print
 * @flags:  calculate the active flags
 * @width: width
